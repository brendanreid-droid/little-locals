...
import { Plus, Edit2, Trash2, LogOut, Search, Calendar, MapPin, Smile, CheckCircle, XCircle } from 'lucide-react';

export default function AdminDashboard() {
  const [events, setEvents] = useState([]);
  const [suggestions, setSuggestions] = useState([]);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState('events'); // 'events' or 'suggestions'
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const suggestionData = suggestionSnapshot.docs.map(doc => ({
          id: doc.id,
          ...doc.data()
        }));
  ...
  const handleApproveSuggestion = async (suggestion) => {
    try {
      ...
      alert("Suggested event approved and added to she's live calendar!");
    } catch (error) {
  ...
  return (
    <div style={{ padding: '40px 20px', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '20px', borderBottom: '1px solid var(--border-soft)', paddingBottom: '24px', marginBottom: '32px' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '0.95rem' }}>
            Promote free Central Coast events and activities.
          </p>
        </div>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          ...
          <button onClick={handleLogout} className="btn btn-outline" style={{ gap: '6px' }}>
      ...
      {/* Tabs Menu */}
      <div style={{ display: 'flex', borderBottom: '2px solid var(--border-soft)', marginBottom: '28px', gap: '20px' }}>
        <button 
      ...
      {loading ? (
        ...
        /* Tab: Suggested Scrapes List (Scraper queue integration) */
        <div>
